...
router.post('/events', async (req, res) => {
    try {
        const { title, description, startTime, endTime, location, isShared } = req.body;
        const userId = req.user.id;
        ...
        const query = `
            ...
            RETURNING *
        `;
        const result = await pool.query(query, [userId, title, description, startTime, endTime, location, isShared || false]);
